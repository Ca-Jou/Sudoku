...
class Interface(Frame):
    ...
    def __init__(self, window):
        ...
        Frame.__init__(self, window, width=100*self.sudoku.getSize(), height=100*self.sudoku.getSize())
        self.pack()

        # Canvas
        self.canvas = Canvas(self, bg='white', width=40+50*self.sudoku.getSize(), height=40+50*self.sudoku.getSize(), highlightthickness=0, bd=0, relief='ridge')
        self.canvas.pack(expand=False, side='left')

        # wonderful background
        unicorn = Image.open("0-Files/kawaii_unicorn.jpg").resize(
            (40+50*self.sudoku.getSize(), 40+50*self.sudoku.getSize()), Image.ANTIALIAS)
